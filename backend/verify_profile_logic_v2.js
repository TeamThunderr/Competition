...
const verifyProfileLogic = async () => {
    ...
    // Corrected logic: no shortlist_verified
    const { data: registrations } = await supabase.from('registrations').select('status, verified, shortlist_proof_url, qualification_verified, competitions(title)').eq('user_id', userId);
    if (!registrations) return;

    // Refined logic matching the fix
    ...
    fs.writeFileSync('verify_result_v2.json', JSON.stringify(result, null, 2));
};
